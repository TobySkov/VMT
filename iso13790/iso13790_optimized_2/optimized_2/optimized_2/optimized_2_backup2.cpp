	...
	const vector<double>& tmp_list)
{
	...
	double Phi__st = (1 - tmp_list[0] - H__tr_w / (9.1*A__t))*(0.5*Phi__int + Phi__sol);

	double local_tmp = Phi__st + H__tr_w * theta__e + H__tr_1 * (theta__sup + (Phi__ia + Phi__HC_nd) / (H__ve));
	double local_tmp2 = C__m / 3600.0;

	//ISO 13790
	//Equations: (C.4), (C.5)
	double Phi__mtot = Phi__m + H__tr_em * theta__e + \
		(H__tr_3 / H__tr_2)*local_tmp;

	double theta__m_t = (theta__m_tm1*(local_tmp2 - (0.5*(H__tr_3 + H__tr_em))) \
		+ Phi__mtot) / (local_tmp2 + (0.5*(H__tr_3 + H__tr_em)));

	double theta__m = 0.5*(theta__m_t + theta__m_tm1);

	double theta__s = (H__tr_ms*theta__m + local_tmp) / (H__tr_ms + H__tr_w + H__tr_1);
...
vector<double> run_sim() {
	...
	vector<double> tmp_list(13);
	fill_tmp_list(tmp_list, A__m, A__t);
